...
use crate::input::{
    ...
    popovers::{ContextMenu, HoverPopover, InputContextMenu},
...
impl InputState {
    ...
    pub(super) fn on_mouse_move(
        ...
        // Show diagnostics inside the main hover popover on mouse move.
        let offset = self.index_for_mouse_position(event.position);
        self.handle_mouse_move(offset, event, window, cx);

        if self.mode.is_code_editor() {
            let diagnostic_range = self
                .mode
                .diagnostics()
                .and_then(|set| set.for_offset(offset))
                .map(|diagnostic| diagnostic.range.clone());
            if let Some(diagnostic_range) = diagnostic_range {
                let current_hover_matches = self
                    .hover_popover
                    .as_ref()
                ...
                if !current_hover_matches {
                    let symbol_range = self
                        ...
                        .unwrap_or_else(|| diagnostic_range.clone());
                    self.hover_popover = Some(HoverPopover::new_diagnostics(
                        cx.entity(),
                        symbol_range,
                        diagnostic_range,
                        cx,
                ...
                .hover_popover
                .as_ref()
                .map(|popover| !popover.read(cx).has_hover())
                .unwrap_or(false)
            {
